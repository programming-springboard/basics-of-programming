...
Иногда в программировании возникает необходимость объявления константных переменных. Понятие константы в данном случае ничем не отличается от аналогичного в математике - такие переменные не могут менять своего значения во времени. Изменить их после того, как вы их объявили будет невозможно. Указывать значение такой переменной необходимо сразу после инициализации:

```c
// Для объявления константы добавляем ключевое слово const перед объявлением
// типа данных.

// Самый банальный пример: число Пи.
const double PI = 3.14;
```

Константы используются для удаления из кода так называемых "магических значений". Чисел или символов, предназначения которых не понятны из контекста. Пример с числом Пи не совсем удачный: со школьной скамьи практически все, увидев `3.14` сказали бы, что это число Пи. Константы уместней всего использовать для каких-то коэффициентов:

```c
double workingHours = 8.0;
double actualWorkingHours = workingHours * 0.2;
```

Согласитесь, не совсем понятно, откуда взялось значение `0.2`. Почему надо умножать именно на него? Что вообще оно скрывает за собой? Все становится проще, когда это значение объявлено константой:

```c
double workingHours = 8.0;
const double workingLossRate = 0.2;
double actualWorkingHours = workingHours * workingLossRate;
```
